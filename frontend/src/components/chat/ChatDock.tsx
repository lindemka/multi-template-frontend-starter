...
import { Paperclip, Smile, Plus, Search, X, MessageSquarePlus } from 'lucide-react'
...
export default function ChatDock() {
        ...
        initialized.current = true
            ; (async () => {
                try {
                    ...
                    const c = await connectChat(async (msg: Message) => {
            ...
            setOpen(true)
                ...
                ; (async () => {
                    try {
                        ...
                        if (!resp.ok) {
                            window.alert('This user is not available for messaging yet.')
                            return
                        }
    ...
    return (
        <div className="fixed bottom-4 right-4 z-50">
            {open && (
                <>
                    <Card className="w-[420px] h-[560px] p-0 flex flex-col shadow-xl rounded-xl overflow-hidden" data-testid="chat-dock">
                        {/* Header */}
                        <div className="flex items-center gap-2 p-3 border-b">
                            <div className="font-semibold text-base flex-1">Messaging</div>
                            <Button variant="secondary" size="sm" className="h-8" onClick={() => { setComposeOpen(true); setComposeQuery('') }}>
                        ...
                        <div className="flex h-full overflow-hidden">
                            {/* Left: conversations */}
                            <div className="w-56 border-r flex flex-col" data-testid="chat-conversations">
                                <div className="p-2">
                                ...
                                <ScrollArea className="flex-1">
                                    <div className="py-1">
                                        {filteredConversations.map((c) => (
                                            <button
                                                key={c.id}
                                                className={`w-full px-3 py-2 hover:bg-accent transition text-left ${active === c.otherUsername ? 'bg-accent' : ''}`}
                            ...
                            {/* Right: thread */}
                            <div className="flex-1 flex flex-col" data-testid="chat-thread">
                                <div className="h-11 flex items-center px-3 border-b">
                                    <div className="font-medium text-sm truncate flex-1">{active || 'Select a conversation'}</div>
                                </div>
                                <ScrollArea className="flex-1 p-3" data-testid="chat-scroll">
            ...
            {!open && (
                <Button onClick={() => setOpen(true)}>Messages</Button>
            )}
        </div>
    )
}
